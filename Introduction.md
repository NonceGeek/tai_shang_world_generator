...
本项目决定从这一点出发，将区块链视为「可验证的自生长的分布式数据源」，直接通过链上无法篡改的数据（如区块数据、哈希数据）作为种子，进行抽象地图的生成。一方面，抽象地图可以进一步被玩家 Mint 成 NFT，衍生出更多玩法；另一方面，各游戏项目方可以讲抽象地图渲染（诠释）为具体游戏地图，直接用在其游戏里。
